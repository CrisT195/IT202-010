...
$query = "SELECT * from Competitions c WHERE expires <= current_timestamp() and calced_winner != 1 and participants >= min_participants";
$db = getDB();
...
if ($r) {
    ...
    if ($comps) {
        ...
        foreach ($comps as $c) {
            ...
            $cid = safe_get($c, "id", -1);
            $query = "SELECT s.user_id, SUM(s.score) as total from Scores s JOIN Usercompetitions uc on s.user_id = uc.user_id WHERE s.created BETWEEN :start AND :end AND uc.competition_id = :cid group by user_id order by total desc limit 3";
            $stmt = $db->prepare($query);
            ...
            if ($r) {
                ...
                $query = "UPDATE Competitions set calced_winner = 1 where id = :cid";
                $stmt = $db->prepare($query);
